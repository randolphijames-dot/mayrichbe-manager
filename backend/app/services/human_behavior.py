...
SelectorLike = Union[str, "Locator"]


def _resolve_locator(page: Page, target: SelectorLike):
    """兼容传入 selector 字符串或 Playwright Locator。"""
    if hasattr(target, "wait_for") and hasattr(target, "click"):
        return target.first
...
async def human_move_and_click(page: Page, selector: SelectorLike, timeout: int = 10000):
    """人类鼠标轨迹：先移动到元素附近，再精确点击"""
    element = _resolve_locator(page, selector)
...
async def human_type(page: Page, selector: SelectorLike, text: str, clear_first: bool = True):
    """模拟人类打字：逐字输入，随机速度，偶尔出错再纠正"""
    element = _resolve_locator(page, selector)

    if clear_first:
        await element.click()
        await asyncio.sleep(0.3)
        platform_name = (await page.evaluate("navigator.platform")).lower()
        await page.keyboard.press("Meta+a" if "mac" in platform_name else "Control+a")
...
async def human_scroll(page: Page, direction: str = "down", distance: Optional[int] = None):
    """模拟人类滚动页面（分段、变速）"""
    if distance is None:
...
class AccountCooldownManager:
    ...
    def _get_history(self, account_id: int) -> list:
        ...
        try:
            ...
            return [
                t.actual_executed_at.replace(tzinfo=timezone.utc)
                if t.actual_executed_at.tzinfo is None
                else t.actual_executed_at
                for t in tasks
            ]
        finally:
            db.close()
