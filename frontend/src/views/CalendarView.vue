              ...
              <div class="flex flex-col gap-1">
                ...
                >
                  <div class="flex items-center justify-between gap-1">
                    <span>{{ dayjs(task.scheduled_at).add(9, 'hour').format('HH:mm') }}</span>
      ...
      <div v-else class="flex flex-col gap-2">
        ...
        >
          <div class="flex flex-col gap-0.5">
            ...
            <span class="text-xs" style="color:var(--text-faint)">{{ dayjs(task.scheduled_at).add(9, 'hour').format('HH:mm') }}</span>
          </div>
...
const calendarCells = computed(() => {
  ...
  for (let d = start; d.isBefore(end) || d.isSame(end, 'day'); d = d.add(1, 'day')) {
    ...
    const dayTasks = tasks.value.filter(t => dayjs(t.scheduled_at).add(9, 'hour').format('YYYY-MM-DD') === dateStr)
...
function getTasksForAccountDate(accountId: number, dateStr: string) {
  return tasks.value.filter(t => {
    return t.account_id === accountId && dayjs(t.scheduled_at).add(9, 'hour').format('YYYY-MM-DD') === dateStr
...
function formatTaskLabel(task: any) {
  ...
  return `${a?.platform === 'instagram' ? '📸' : '▶'} ${dayjs(task.scheduled_at).add(9, 'hour').format('HH:mm')}`
}
